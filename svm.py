...
def trainClassifier():
    ...
    target = np.array([x for x in range(10) for i in range(4800)])
    ...
    parameters = {
        ...
        'linearsvc__class_weight': [{0:a, 4:b, 6:c, 7:d} for a in [0.8,1.2,1.6]
                                    for b in [0.8,1.2,1.6] for c in [0.8,1.2,1.6] for d in [0.8,1.2,1.6]],
    }
...
def testClassifier():
    ...
    target = np.array([x for x in range(10) for i in range(1000)])

    # 读取分类器
    classifier = joblib.load(classifier_path)
    predicted = classifier.predict(matrix)

    term_file_folder_path = 'cnews-test/'
